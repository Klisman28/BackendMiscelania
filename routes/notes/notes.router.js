...
router.get('/',
  async (req, res, next) => {
    try {
      ...
      const data = await service.find(req.query, req.companyId);
...
router.post('/',
  ...
  async (req, res, next) => {
    try {
      const body = req.body;
      const newNote = await service.create(body, req.companyId);
...
router.put('/:id',
  ...
  async (req, res, next) => {
    try {
      ...
      const updatedNote = await service.update(id, changes, req.companyId);
...
router.delete('/:id',
  ...
  async (req, res, next) => {
    try {
      const { id } = req.params;
      const rta = await service.delete(id, req.companyId);
      success(res, rta);
    } catch (error) {
      next(error);
    }
